...
def load_graph_from_api(name):
    ...
    resp = requests.get(TWINS_ENDPOINT + "/twins/urn:test", headers=headers)
    resp.raise_for_status()
...
def execute_test():
    ...
    set_property("urn:test:GateA1", "occupied", True)
    set_property("urn:test:GateA2", "occupied", True)
    set_property("urn:test:GateB1", "occupied", True)
    set_property("urn:test:GateB2", "occupied", False)
    ...
    set_property("urn:test:GateB2", "occupied", True)
    ...
    print("Expected: NOT COLLAPSED")
    setPlane("urn:test:GateA1", "urn:test:Plane1")
    setPlane("urn:test:GateA2", "urn:test:Plane2")
    setPlane("urn:test:GateB1", "urn:test:Plane3")
    ...
    print("Expected: COLLAPSED")
    setPlane("urn:test:GateB2", "urn:test:Plane4")
    ...
    print("Expected: NOT COLLAPSED")
    set_property("urn:test:Plane1", "flying", False)
    set_property("urn:test:Plane2", "flying", False)
    set_property("urn:test:Plane3", "flying", False)
    set_property("urn:test:Plane4", "flying", True)
    set_property("urn:test:Plane5", "flying", True)
    g = load_graph_from_api("esc5")
    querys.check_p2p(g)
    
    print("\nScenario 6: Updating Plane5 flying status to False and re-checking peer-to-peer relationships...")
    print("Expected: COLLAPSED")
    set_property("urn:test:Plane5", "flying", False)
